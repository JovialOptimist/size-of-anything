...
const ActiveElementDisplay: React.FC = () => {
  ...
  const setSatelliteSnapshot = useMapStore(
    (state: any) => state.setSatelliteSnapshot
  );

  const colorPickerRef = useRef<HTMLDivElement>(null);
  const satelliteSnapshotOn = Boolean(activeElement?.properties?.satelliteSnapshot);
    ...
    <div className="active-element-display active-element-panel">
      <div className="element-info">
        ...
        <div className="button-group">
          ...
          <button
            className={`icon-action-button ${satelliteSnapshotOn ? "satellite-snapshot-on" : ""}`}
            onClick={() => activeAreaId && setSatelliteSnapshot(activeAreaId, !satelliteSnapshotOn)}
            title={satelliteSnapshotOn ? "Hide satellite snapshot" : "Show satellite imagery of this area (moves with shape)"}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
